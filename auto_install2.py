...
class Ui:
    ...
    def auto_install(self):
        ...
        for each in menu:
            ...
            if each == 'baidu_Netdisk':
                ...
                png_file_name = "_shot"
                result = install_from_png(app_name=each, edit_index=0, png_file_name=png_file_name,
                ...
                if result:
                    txt_change(prom_name=each, menu_change=menu_change)
                    kill_program(title='欢迎使用百度网盘', backend='uia')
                else:
    ...
    def ui(self):
        ...
        tk.Label(self.root, text="安 装 路 径：").grid(
            row=0, column=0, padx=0, pady=4)  # 纯文字显示

        tk.Label(self.root, textvariable=self.path).grid(
            row=0, column=1, padx=0, pady=4)  # 显示路径的label

        tk.Button(self.root, text="更改目录", command=self.change_dir).grid(
